...
import net.mullvad.map.data.LocationMarkerColors
...
private fun MapPreview() {
    ...
    InteractiveMap(cameraPosition = CameraPosition(latLong = berlin, zoom = 1.9f))
}

@Preview
@Composable
private fun SpinningMapPreview() {
...
private fun InteractiveMapPreview() {
    ...
    val longitudeAnimatable = remember { Animatable(selectedLocation.longitude.value) }
    val latitudeAnimatable = remember {
        Animatable(selectedLocation.latitude.value).also { it.updateBounds(-40f, upperBound = 60f) }
    }
    val userZoomAnimatable = remember { Animatable(1.2f).also { it.updateBounds(1.2f, 2f) } }
    ...
    LaunchedEffect(selectedLocation) {
        ...
        val currentPosition =
            LatLong(
                Latitude.fromFloat(latitudeAnimatable.value),
                Longitude.fromFloat(longitudeAnimatable.value),
            )
        val distance = selectedLocation.seppDistanceTo(currentPosition)
        val duration = distance.toAnimationDurationMillis()
        launch {
            longitudeAnimatable.animateTo(
                selectedLocation.longitude.value,
                animationSpec = tween(duration),
            )
        }
        launch {
            latitudeAnimatable.animateTo(
                selectedLocation.latitude.value,
                animationSpec = tween(duration),
            )
        }
        launch { userZoomAnimatable.animateTo(targetValue = 1.2f, animationSpec = tween(duration)) }
    }
    ...
    InteractiveMap(
            ...
            Modifier.pointerInput(Unit) {
                detectTransformGesturesWithEnd(
                    ...
                    onGestureStart = {
                        ...
                        scope.launch { longitudeAnimatable.stop() }
                        scope.launch { latitudeAnimatable.stop() }
                    },
                    onGesture = { centroid: Offset, pan: Offset, newZoom: Float, rotation: Float ->
                        val longitude =
                            longitudeAnimatable.value - pan.x * userZoomAnimatable.value / 50f
                        val tempLatitude = Latitude.fromFloat(latitudeAnimatable.value)
                        val latitude = (tempLatitude.value + pan.y * userZoomAnimatable.value / 40f)
                        val newZoom = (userZoomAnimatable.value + (1 - newZoom) * 0.5f)

                        tracker.addPosition(System.currentTimeMillis(), Offset(longitude, latitude))

                        scope.launch {
                            userZoomAnimatable.snapTo(newZoom)
                            longitudeAnimatable.snapTo(Longitude.fromFloat(longitude).value)
                            latitudeAnimatable.snapTo(latitude)
                        }
                    },
                    onGestureEnd = {
                        val velocity = tracker.calculateVelocity()
                        val decay = exponentialDecay<Float>(frictionMultiplier = 1f)
                        scope.launch { longitudeAnimatable.animateDecay(velocity.x, decay) }
                        scope.launch {
                            var velY = velocity.y
                            do {
                                val res = latitudeAnimatable.animateDecay(velY, decay)
                        ...
                        idleInteractionJob =
                            scope.launch {
                                ...
                                launch { userZoomAnimatable.animateTo(1.15f, animationSpec) }
                            }
                    },
                )
            },
        cameraPosition =
            CameraPosition(
                ...
                verticalBias = 0.5f,
                zoom = userZoomAnimatable.value,
            ),
        markers = markers,
        onClickRelayItemId = {
            selectedLocation =
                locations.firstOrNull { loc -> loc == it.latLong } ?: selectedLocation
        },
    )
}
